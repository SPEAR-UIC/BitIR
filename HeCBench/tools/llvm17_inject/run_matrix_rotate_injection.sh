...
RUN_DUMP="${RESULTS_DIR}/matrix-rotate_${MATRIX_SIZE}_${MATRIX_REPEAT}_site${SITE_ID}_bit${BIT_INDEX}.bin"
...
set +e
HECBENCH_LLFI_FORCE_DUMP=1 "${BIN_PATH}" "${MATRIX_SIZE}" "${MATRIX_REPEAT}" "${RUN_DUMP}" >"${RUN_OUT}" 2>"${RUN_ERR}"
status=$?
set -e
...
if [[ ${status} -eq 0 ]]; then
  if [[ -f "${RUN_DUMP}" ]]; then
    if [[ "${BASELINE}" -eq 1 ]]; then
      cp -f "${RUN_DUMP}" "${BASELINE_PATH}"
      echo "[inject] baseline_written=${BASELINE_PATH}" >>"${RUN_OUT}"
      result="BASELINE"
    else
      if python3 "${COMPARE_TOOL}" "${GOLDEN}" "${RUN_DUMP}" --abs-tol "${ABS_TOL}" --rel-tol "${REL_TOL}" >>"${RUN_OUT}" 2>>"${RUN_ERR}"; then
        result="MASKED"
      else
        result="SDC"
      fi
...
echo "${SITE_ID},${BIT_INDEX},${result},${status},${RUN_OUT},${RUN_ERR},${RUN_DUMP}" >> "${CSV}"
