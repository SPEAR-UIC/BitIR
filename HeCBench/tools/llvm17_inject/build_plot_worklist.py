...
def main():
    ...
    p.add_argument('--clang', default='/soft/compilers/llvm/release-17.0.6/bin/clang++')
    p.add_argument('--llvm-as', default='/soft/compilers/llvm/release-17.0.6/bin/llvm-as')
    p.add_argument('--opt', default='/soft/compilers/llvm/release-17.0.6/bin/opt')
    p.add_argument('--cuda-home', default='/soft/compilers/cudatoolkit/cuda-11.8.0')
    p.add_argument('--cuda-arch', default='sm_80')
    p.add_argument('--out-dir', default='HeCBench/build/llvm17-inject-bench-worklist')
    p.add_argument('--target', choices=['result','operand','pointer'], default='result')
    p.add_argument('--int-float-only', type=int, choices=[0,1], default=1)
    p.add_argument('--include-constants', type=int, choices=[0,1], default=0)
    args = p.parse_args()
    ...
    sites_path = os.path.join(results_dir, f'sites_plot{suffix}.csv')
    ...
    worklist_path = os.path.join(results_dir, f'worklist_plot{suffix}.csv')
    with open(worklist_path, 'w', encoding='utf-8') as wl:
        wl.write('index,site_id,bit_index,bitwidth,type_kind,opcode,function\n')
        index = 0
        with open(sites_path, 'r', encoding='utf-8') as fh:
            ...
            for line in fh:
                ...
                function = parts[5] if len(parts) > 5 else 'UNK'
                ...
                for bit in range(bitwidth):
                    index += 1
                    wl.write(f"{index},{site_id},{bit},{bitwidth},{kind},{opcode},{function}\n")
