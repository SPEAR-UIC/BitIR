...
BASE_ENV = {
    ...
    'TRACE_LEVEL': os.environ.get('SUPPLEMENTAL_TRACE_LEVEL', 'off'),
...
def expected_trace_dir(bench: str, site: str, bit: str, trial: str, baseline: bool):
    bench_dir = FULL_DUMP_ROOT / 'local_dumps' / bench
    if baseline:
        suffix = '' if str(trial) == '1' else f'_trial{trial}'
        return bench_dir / f'trace_baseline{suffix}'
    suffix = '' if str(trial) == '1' else f'_trial{trial}'
    return bench_dir / f'trace_site{site}_bit{bit}{suffix}'
...
def write_all(golden_rows, timeout_rows, baseline_rows, trial_rows, hash_rows, diag_rows, repro_rows):
    ...
    write_csv(RESULTS_DIR / 'per_trial_results.csv', ['screen_id', 'bench', 'old_site_id', 'site_id', 'bit_index', 'trial', 'classification', 'due_reason', 'runner_returncode', 'summary_result', 'summary_exit_code', 'dump_exists', 'dump', 'compare_json', 'stdout', 'stderr', 'trace_dir'], trial_rows)
        ...
        if status != 'PASS':
            for row in bench_rows:
                for trial in range(1, int(row.get('trials', '2') or '2') + 1):
                    trial_rows.append({'screen_id': row['screen_id'], 'bench': bench, 'old_site_id': row['old_site_id'], 'site_id': row['site_id'], 'bit_index': row['bit_index'], 'trial': trial, 'classification': 'SKIPPED_BASELINE_MISMATCH', 'due_reason': '', 'runner_returncode': '', 'summary_result': '', 'summary_exit_code': '', 'dump_exists': False, 'dump': '', 'compare_json': '', 'stdout': '', 'stderr': '', 'trace_dir': ''})
        ...
        for row in bench_rows:
            ...
            for trial in range(1, int(row.get('trials', '2') or '2') + 1):
                ...
                trial_rows.append({
                    ...
                    'trace_dir': str(expected_trace_dir(bench, row['site_id'], row['bit_index'], str(trial), False)),
                })
                diag_rows.append({
                    'screen_id': row['screen_id'],
                    'bench': bench,
